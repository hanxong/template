    ...
    $.fn[pluginName] = function (options) {
      ...
      return this.each(function () {
        var elem = this,
          $elem = $(elem),
          $input =
            jQuery.type(options.inputField) == "string"
              ? $(options.inputField)
              : options.inputField,
          $form = $input.parents("form").length
            ? $($input.parents("form")[0])
            : $elem;

        var numbers = Array.apply(null, Array(9)).map(function (_, i) {
          return $(options.buttonTemplate)
            .html(i + 1)
            .addClass("number");
        });
        numbers.push(
          $(options.buttonTemplate)
            .html(options.resetButtonText)
            .addClass(options.resetButtonClass)
        );
        numbers.push($(options.buttonTemplate).html("0").addClass("number"));
        numbers.push(
          $(options.buttonTemplate)
            .html(options.deleteButtonText)
            .addClass(options.deleteButtonClass)
        );
        ...
        $elem.find(".number").click(function (e) {
          ...
          if (inputValue.length < 11) {
            $input.val(inputValue + $(e.target).text());
        ...
        $elem.find("." + options.resetButtonClass).click(function (e) {
          $("#numInput").val("");
        });
      });
    };
